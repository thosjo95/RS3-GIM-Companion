...
- 🔍 **Skill spotlight** — searchable input above the Skills matrix spotlights any skill: all other rows fade to ~20% opacity while the selected row gets a gold tint; a live leaderboard banner shows 🥇🥈🥉 rankings with each player's level in their colour.
- 👥 **Custom Groups** — promoted to production; regular RS3 players can now create a tracking group without a GIM account. Appears in both the Setup screen and the Find / Manage Groups modal for all users.
